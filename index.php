...
function renderZendPanel(?string $zendLogoData, ?string $zendBlurb): ?string
{
    ...
    $logoData = $zendLogoData ?? ZEND_ENGINE_LOGO_FALLBACK;
    $logoMarkup = '';
    if ($logoData !== null) {
        $logoMarkup = sprintf(
            '<img src="%s" alt="Zend Engine Logo" class="phpinfo-zend__logo" />',
            htmlspecialchars($logoData, ENT_QUOTES)
        );
    }

    $messageMarkup = '';
    if ($zendBlurb !== null && trim($zendBlurb) !== '') {
        $normalized = html_entity_decode((string) $zendBlurb, ENT_QUOTES | ENT_HTML5);
        ...
        if ($normalized !== '') {
            $messageMarkup = '<div class="phpinfo-zend__text">' . nl2br(htmlspecialchars($normalized, ENT_QUOTES)) . '</div>';
        }
    }

    if ($logoMarkup === '' && $messageMarkup === '') {
        return null;
    }

    $headerPieces = [];
    $headerPieces[] = '<h3 class="phpinfo-zend__title">Zend Engine &amp; Loader</h3>';
    if ($logoMarkup !== '') {
        $headerPieces[] = '<div class="phpinfo-zend__brand">' . $logoMarkup . '</div>';
    }

    $headerMarkup = '<header class="phpinfo-zend__header">' . implode('', $headerPieces) . '</header>';
    $bodyMarkup = $messageMarkup !== ''
        ? '<div class="phpinfo-zend__body">' . $messageMarkup . '</div>'
        : '';

    return '<div class="phpinfo-zend">' . $headerMarkup . $bodyMarkup . '</div>';
}
...
function buildPhpInfoMarkup(): string
{
    ...
    $phpinfo = (string) preg_replace('%<h1([^>]*)>%i', '<h2$1>', $phpinfo);
    $phpinfo = (string) preg_replace('%</h1>%i', '</h2>', $phpinfo);
...
$stylesVersion = (string) (filemtime(__DIR__ . '/assets/styles.css') ?: 0);
$scriptVersion = (string) (filemtime(__DIR__ . '/assets/app.js') ?: 0);
?>
<!DOCTYPE html>
<html lang="de">
  <head>
    ...
    <script type="module" src="assets/app.js?v=<?= $scriptVersion ?>" defer></script>
  </head>
  <body>
    <main>
